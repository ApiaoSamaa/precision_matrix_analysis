...
if __name__ == '__main__':
    ...
    # 导入训练数据
    for e in [0.01,0.03,0.04,0.06, 0.07]:
        for lam in [0.01, 0.03, 0.04, 0.06]:
            load_file_name = "./process/graphs/graph_"+str(iter)+"_eps"+str(e)+"lam"+str(lam)
            ...
            # for num in range(int(np.sum(mask_matrix)), int(np.sum(mask_matrix))+10, 1):
                ...
                filename = "./process/fasts/"+str(iter)+"/fastmdmc_input/dimension"+str(p)+"nonzero" +str(nonzero_edges) + "fastmdmc_input_cov.json"
